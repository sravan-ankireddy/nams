...
from channel_ae import Channel_AE, Channel_AE_non_trainable
...
def import_enc(args):
    from encoders import ENC_interCNN_bi_interleaver as ENC
    return ENC

def import_dec(args):
    from decoders import DEC_LargeCNN_bi_interleaver as DEC
    return DEC
...
if __name__ == '__main__':
    ...
    model_grid = Channel_AE_non_trainable(args, encoder, decoder).to(device)

    model_grid.args = args
    
    
    
    # make the model parallel
    if args.is_parallel == 1:
        model_grid.enc.set_parallel()
        model_grid.dec.set_parallel()

        

    ##Load enc/dec from grid
    # pretrained_model = torch.load("./results_init_intrlvr/grid_trained_on_awgn_bl40.pt")
    # model_grid.load_state_dict(pretrained_model)
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        if epoch%5==0:
            torch.save(model.state_dict(), './results_init_intrlvr/bi_int_no_init_grid.pt')
