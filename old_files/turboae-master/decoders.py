...
class DEC_LargeCNN(torch.nn.Module):
    ...
    def forward(self, received, interleaver):
        ...
        for idx in range(self.args.num_iteration - 1):
            ...
            #prior      = self.deinterleaver(x_plr) ###DO SOMETHING HERE
        ...
        #final      = torch.sigmoid(self.deinterleaver(x_plr))   ###DO SOMETHING HERE

        return final


##2 interleavers, 1 grid then 1 trainable
class DEC_LargeCNN_bi_interleaver(torch.nn.Module):
    def __init__(self, args):
        super(DEC_LargeCNN_bi_interleaver, self).__init__()
        self.args = args
    ...
    def forward(self, received, interleaver):
        ...
        r_sys     = received[:,:,0].view((self.args.batch_size, block_len, 1))
        r_sys_int = self.interleaver(r_sys)
        r_sys_int = interleaver(r_sys_int)
        r_par1    = received[:,:,1].view((self.args.batch_size, block_len, 1))
        r_par2    = received[:,:,2].view((self.args.batch_size, block_len, 1))
        ...
        for idx in range(self.args.num_iteration - 1):
            ...
            x_plr_int  = self.interleaver(x_plr)
            x_plr_int  = interleaver(x_plr_int)
            ...
            prior = torch.matmul(interleaver.perm_matrix.weight.t(), x_plr)
            prior      = self.deinterleaver(prior) ###DO SOMETHING HERE
        ...
        final = torch.sigmoid(torch.matmul(interleaver.perm_matrix.weight.t(), x_plr))
        final      = torch.sigmoid(self.deinterleaver(final))   ###DO SOMETHING HERE
