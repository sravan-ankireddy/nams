...
def run_sim(h,g,steps,lr,relu,nn_eq,quant_wt,max_iter,coding,chan,alpha,eb_n0_lo,eb_n0_hi,testing_batch_size,offline_test,models,adapt,freeze,ff):
	...
	# max_iter = "5"
	# os.system(" CUDA_VISIBLE_DEVICES=0,1,2,3 python neural_ms.py -save_ber_to_mat 1 -use_offline_testing_data " + offline_test + " -entangle_weights -1 -steps " + steps + " -learning_rate " + lr + " -adaptivity_training " + adapt + " -relu " + relu  + " -nn_eq " + nn_eq + " -num_iterations " + max_iter + " -coding_scheme " + coding + " -channel_type " + chan + " -alpha " + alpha + " -gpu_index " + gpu_id[0] + " -decoder_type spa -testing_batch_size " + testing_batch_size_bp + " -eb_n0_lo " + eb_n0_lo + " -eb_n0_hi " + eb_n0_hi + " -H_filename "+ h + " -G_filename " + g +\
	# " & CUDA_VISIBLE_DEVICES=0,1,2,3 python neural_ms.py -save_ber_to_mat 1 -use_offline_testing_data " + offline_test + " -entangle_weights -2 -steps " + steps + " -learning_rate " + lr + " -adaptivity_training " + adapt + " -relu " + relu  + " -nn_eq " + nn_eq + " -num_iterations " + max_iter + " -coding_scheme " + coding + " -channel_type " + chan + " -alpha " + alpha + " -gpu_index " + gpu_id[1] + " -decoder_type min_sum -testing_batch_size " + testing_batch_size_bp + " -eb_n0_lo " + eb_n0_lo + " -eb_n0_hi " + eb_n0_hi + " -H_filename "+ h+" -G_filename "+ g )
...
models_folder_main = "saved_models_cv/alpha_interf_0.2/arch_1/"
...
for nn_eq in nn_eq_list:
	if (adapt == "0"):
		...
		steps_list = ["5000"]#["20000","15000","10000","5000","2000"]
	else:
				...
				for coding_scheme in coding_scheme_list:
					for channel_type in channel_type_list:
						if (coding_scheme == "BCH"):
							...
							for im in range(7):
								model_prefix = models_folder_main + "ent_" + str(im) + "/lr_" + lr + "/intermediate_models/nams_" + prefix + "_st_" + steps + "_lr_" + lr + "_" + chan_tr + "_ent_"
								filename = model_prefix + str(im) + model_suffix
								models.append(filename)
							# eb_n0_lo = "12"
							# eb_n0_hi = "16"
							run_sim(h,g,steps,lr,relu,nn_eq,quant_wt,max_iter,coding_scheme,channel_type,alpha,eb_n0_lo,eb_n0_hi,testing_batch_size,offline_ts_data,models,adapt,freeze,ff)
