...
if (channel == "ETU")
    data = load("data_files/lte_data/BCH_63_36_ETU_df_0_data_train_5_22_all_z.mat");
    llr_ch_etu = data.llr(:,1:num_frames,:);
    enc_etu = data.enc(:,1:num_frames,:);
end
...
for i_SNR = 1:length(SNR_dB)
    ...
    parfor i_frame = 1:num_frames
        ...
        if (channel == "AWGN")
            ...
            % BPSK demodulation
            llr = 2*y_nms/(sig^2);
            [llr_out_nms, llr_updates, llr_updates_full] = weighted_min_sum_gs(llr,H,max_iter);
...
%% Emp distribuition : Gaussian approx for entangled model

% extract data for node i_n
W_ana_gw_ent_2 = zeros(N,length(Eb_N0_dB));
...
for i_SNR = 1:length(Eb_N0_dB)
    ...
    for i_n = 1:N
        ...
        LB = 0;
        UB = 1;
        [xmin,fmin] = fminsearchbnd(f,x0,LB,UB);
        
        W_ana_gw_ent_2(i_n,i_SNR) = xmin;
    end
end

% reshape to size (H)
% get ber plots
W_ana_gw_ent_2_mat = zeros(size(H,1),size(H,2),length(Eb_N0_dB));
for i_s = 1:length(Eb_N0_dB)
    W_ana_gw_ent_2_mat(:,:,i_s) = repmat(W_ana_gw_ent_2(:,i_s)',size(H,1),1).*H;
end

figure;
for i_s = 1:length(Eb_N0_dB)
    plot(W_ana_gw_ent_2(:,i_s));
    hold on;
end

%% Emp distribuition : Gaussian approx for disentangled model

% For each edge connected to each variable node, find the empirical
% distribuition

% extract data for node i_n
W_ana_gw_ent_1_mat = zeros(N-K,N,length(Eb_N0_dB));
for i_SNR = 1:length(Eb_N0_dB)
    disp(i_SNR);
    % extract iter 1 data
    ...
    for i_n = 1:N
       ...
       % get mean and var of all edges
        ...
        [W_ana_gw_ent_1_mat(:,i_n,i_SNR),fmin] = fminsearchbnd(f,x0,LB,UB);
    end
end

figure;
for i_s = 1:length(Eb_N0_dB)
    temp = (W_ana_gw_ent_1_mat(:,:,i_s).*H)';
    temp = temp(temp~=0);
    plot(temp);
    hold on;
end

W_ana_gw_ent_1_mean = zeros(N,length(Eb_N0_dB));

for i_e = 1:length(Eb_N0_dB)
    for i_n = 1:N
        nz_ind = H(:,i_n) > 0;
        W_ana_gw_ent_1_mean(i_n,i_e) = mean(W_ana_gw_ent_1_mat(nz_ind,i_n,i_e));
    end
end

figure;
for i_e = 1:length(Eb_N0_dB)
    plot(W_ana_gw_ent_1_mean(:,i_e));
    hold on;
end

%% BER simulations using Gaussian weights -- ent 2

% load test data
if (channel == "ETU")
    data = load("data_files/lte_data/BCH_63_36_ETU_df_0_data_test_5_22.mat");
    llr_ch_etu = data.llr(:,1:num_frames,:);
    enc_etu = data.enc(:,1:num_frames,:);
end
...
W_nn_gw_ent_2 = W_gw;
plot(W_nn_gw_ent_2);
legend("Weights from Gaussian Approximation","Weights from NNMS");
title("Gaussian vs. NN weights - Ent 2");

f = figure;
plot(W_ana_gw_ent_2(:,4),'-.*');
hold on;
plot(W_nn_gw_ent_2,'--o');

ylabel("Weights");

xlabel("Variable node index");

leg = legend('Weights from Gaussian Approximation', 'Weights from NNMS');
...
% reshape to size (H), length(SNRdB)
% get ber plots
W_nn_gw_ent_2_mat = repmat(repmat(W_nn_gw_ent_2,size(H,1),1),1,1,length(Eb_N0_dB));
W_nn_cur = W_nn_gw_ent_2_mat;
B_nn_cur = zeros(size(W_nn_cur));

% rep ana wts across var nodes
W_ana_gw_ent_2_mat = permute(repmat(W_ana_gw_ent_2,1,1,size(H,1)),[3 1 2]);
W_ana_mat = W_ana_gw_ent_2_mat;

% compare BER performance
simulate_ber_gauss_vs_nn;
title("BER : gaussian vs NN - Ent 2");
grid on;
ax = gca;
fs = 28;
set(gca,'FontSize',fs);
...
for i_col = 1:size(H,2)
    for i_row = 1:size(H,1)
        if (H(i_row,i_col) > 0)
            ...
            W_nn_gw_ent_1_mat(i_row,i_col) = W_nn_gw_ent_1(edge_count);
        end
    end
end

W_nn_gw_ent_1_mean = zeros(N,1);
for i_n = 1:N
    nz_ind = H(:,i_n) > 0;
    W_nn_gw_ent_1_mean(i_n,1) = mean(W_nn_gw_ent_1_mat(nz_ind,i_n));
end

f = figure;
plot(W_ana_gw_ent_1_mean(:,8),'-.*');
hold on;
plot(W_nn_gw_ent_1_mean,'--o');
...
W_ana_mat = W_ana_gw_ent_1_mat;

% replicate across SNRs
W_nn_gw_ent_1_mat = repmat(W_nn_gw_ent_1_mat,1,1,length(SNR_dB));
W_nn_cur = W_nn_gw_ent_1_mat;
B_nn_cur = zeros(size(W_nn_cur));

simulate_ber_gauss_vs_nn;
title("BER : gaussian vs NN - Ent 1");
grid on;
ax = gca;
fs = 28;
set(gca,'FontSize',fs);
