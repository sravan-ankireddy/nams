...
class NeuralNetwork(nn.Module):
	def __init__(self):
		...
		# Gaussian weights at each variable node
		num_wgs = n
		W_gw_init = torch.ones(1, num_wgs).to(device)
...
def marginalize(soft_input, cv, batch_size):
	...
	if 0:
		...
		soft_output = torch.tile(torch.reshape(model.W_gw,[-1,1]),[1,batch_size])* soft_output + weighted_soft_input
	else:
	...
	while step < steps:
		...
		if (args.save_torch_model == 1 and step % saver_step == 0):
			...
			if (args.cv_model == 1 and args.vc_model == 0):
				...
				weights = {'B_cv':B_cv, 'W_cv':W_cv, 'W_gw':W_gw}
	...
	if (step > 0):
		...
		if (args.save_torch_model == 1):
			...
			if (args.cv_model == 1 and args.vc_model == 0):
				...
				W_gw_temp = model.W_gw.cpu().data.numpy()
				weights_temp = {'B_cv':B_cv_temp, 'W_cv':W_cv_temp, 'W_gw':W_gw_temp}
			elif (args.cv_model == 0 and args.vc_model == 1):
