function [desired_peaks,echo_width,desired_peaks_bsb,echo_width_bsb,cancel_mean,cancel_mean_f,cancel_max_f,mode,desired_peaks_judge,echo_width_judge,residual_mean_f] = cancel_wrapper(ht_bsb,ht_bsb_dc,ht_down,opt_taps_t,params,sys_params)
    ...
    % echo coarse delay
     ...
     params.peak_th          = 0.27;%0.35;%0.39;
     params.idx_neighbors_th = 3;%3;
     params.small_width_th   = 2;
     
     [desired_peaks,echo_width,total_pow,peaks_range] = detect_coarse_delay(ht_bsb_dc,params);
     ...
     [desired_peaks_bsb,echo_width_bsb,total_pow_bsb] = detect_coarse_delay(ht_down,params);
    ...
    if length(idx_peaks) >= 2
        ...
        peaks_range_judge   = peaks_range(idx_peaks(1:2),:);
    ...
    if single_wide_peak == 1 % check if it is actually two separable peaks 
        disp('check if two seperate peaks can be determined ')
...
%         desired_peaks = round(desired_peaks);        
         ...
         ht_temp(peaks_range_judge(1):peaks_range_judge(2)) = ht_bsb_dc(peaks_range_judge(1):peaks_range_judge(2));
         
         th = 0 ; % threshold not used
         desired_peaks = find_peak_by_diff(abs(ht_temp),th);
         
         peaks_value = abs(ht_temp(desired_peaks));
         %[v idx_peaks] = sort(total_pow,'descend');
         [v idx_peaks] = sort(peaks_value,'descend');
          
          
          if length(idx_peaks) >= 2
              desired_peaks_judge = desired_peaks(idx_peaks(1:2)) % choose the largest two peaks
