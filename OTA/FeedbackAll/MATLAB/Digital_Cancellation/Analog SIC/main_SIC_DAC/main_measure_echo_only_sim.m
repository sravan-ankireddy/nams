% simulation of echo channel measurement only
clear all;
close all;
...
%-------- simulated echo channel ------------
echo_ch = cell(1,3);
echo_ch{1} = [zeros(1,50) 1 zeros(1,50) 0.1];
echo_ch{2} = [zeros(1,50) 1 ];
echo_ch{3} = [zeros(1,50) 1 ];
%-------------------------------------------

% echo signal 
params.filter_coe    = echo_ch;              % baseband filter coefficient for echo emulation. Each channel has different coefficient  
...
for idx_sym = 1:N_sym_est
    ...
    [ht_rf{idx_sym},hf_rf{idx_sym},hf_bsb{idx_sym},ht_bsb{idx_sym},flag_recapture] = multi_chest_wrapper(dt_bsb_sub,y_echo,OFDM_params,sys_params,isbsb,N_channel);
end
...
%--------------- test baseband channel estimation ------------
isbsb = 1;
idx_sym = 1;
idx_ch = 1;
N_channel = 1;
...
h = [zeros(1,50) 1 zeros(1,50) 0.05 zeros(1,50) 1e-3];

dt_bsb_sub = cell(1,N_channel);
for idx_ch = 1:N_channel
    dt_bsb_sub{idx_ch} = dt_bsb{idx_sym,idx_ch};
      echo_bsb = conv(h,[dt_bsb_sub{idx_ch} dt_bsb_sub{idx_ch}]);
end    
[ht_rf{idx_sym},hf_rf{idx_sym},hf_bsb{idx_sym},ht_bsb{idx_sym},flag_recapture] = multi_chest_wrapper(dt_bsb_sub,echo_bsb,OFDM_params,sys_params,isbsb,N_channel);
