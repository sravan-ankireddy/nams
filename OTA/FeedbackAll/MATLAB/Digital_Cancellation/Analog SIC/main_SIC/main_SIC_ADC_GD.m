...
for idx_branches = 1:N_branches  
    ...
    step = 0.01;
    coe = coe_pre + step*gradient;
    %-------------------------
    
    if idx_ite == 1
      coe_mem{idx_branches} = coe{idx_branches};
    else
      coe_mem{idx_branches} = coe_mem{idx_branches} + coe{idx_branches};        
    end

    code = find_coe_to_code(real(abs(coe_mem{idx_branches})),code_table,code_to_coe_table);
    coe{idx_branches}
    uint32(code)
    code_v = 5/2^16*code
