...
label1 = uilabel(fig);
    ...
    label1.Position =[100 384 100 15];
    label1.FontSize     = 22;
    label1.Text     = 'GUI CONTROL FOR WIRELESS DEMO';

% Create a push button
sta = uibutton(fig,'push',...
               'Text','CANCELATION','Position',[220, 290, 100, 22],...
               'ButtonPushedFcn', @(sta,event) start(sta));
           
stp = uibutton(fig,'push',...
               'Text','SIC OFF','Position',[220, 190, 100, 22],...
               'ButtonPushedFcn', @(stp,event) stop(stp)); 

res = uibutton(fig,'push',...
               'Text','RESUME','Position',[220, 90, 100, 22],...
               'ButtonPushedFcn', @(res,event) resume(res)); 
...
%         disp(str);
        ...
        for sign_trial = [1,-1]
           ...
           % ------------ Measure cancelation------------------------------ 
            ...
            if sic == 0 
                hold off;
                plot(f/1e6,to_pow_dB(echo),'LineWidth',2);
                
                hold on;
                plot(f/1e6,to_pow_dB(echo)+0.15,'LineWidth',2);
                Cancel_Mag(k) = 0;
            end
...
%             cancel_perc = (length(Cancel_Mag(Cancel_Mag>30))/length(Cancel_Mag))*100;
%             clc;
%             display('Percentage above Threshold:',num2str(cancel_perc));
          ...
          elseif cancel < 25 && sic ==1
             ...
             if sic == 0 
                 ...
                 hold off;
                plot(t,to_pow_dB(echo_t),'LineWidth',2);
                
                hold on;
                plot(t,to_pow_dB(echo_t)+0.15,'LineWidth',2);
